...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gmd="http://www.isotc211.org/2005/gmd" 
	...
	<!-- SMR 2017-10-15 modifiy keyword presentation to include text in keywordTypeCode if it is different from 
	the codeListValue -->
		...
		<dd>
			<xsl:choose>
				...
				<xsl:when test="gmd:type/gmd:MD_KeywordTypeCode[@codeListValue='005' or @codeListValue='theme']">
					<xsl:choose> 
						<xsl:when test="gmd:type/gmd:MD_KeywordTypeCode/text()!= '' 
							and gmd:type/gmd:MD_KeywordTypeCode/@codeListValue != gmd:type/gmd:MD_KeywordTypeCode/text()">
					<dt>
						<span class="element">
							<xsl:value-of select="concat(gmd:type/gmd:MD_KeywordTypeCode/text(), ' Theme keywords:')"/>
						</span>
					</dt>
						</xsl:when>
					<xsl:otherwise>
				...
				<xsl:otherwise>
					<xsl:choose> 
						<xsl:when test="gmd:type/gmd:MD_KeywordTypeCode/text()!= '' 
							...
							<dt>
								<span class="element">
									<xsl:value-of select="concat(gmd:type/gmd:MD_KeywordTypeCode/text(), ' ',gmd:type/gmd:MD_KeywordTypeCode/@codeListValue,' keywords:')"/>
								</span>
							</dt>
						</xsl:when>
						<xsl:otherwise>
...
<!--						<dt>-->
  ...
  <xsl:choose>
	...
	<!-- gco:CharacterString, gmx:Anchor , gco:FreeText -->
	<!-- smr 20171015 add test for anchor -->
	<xsl:template name="CharacterString">
		<xsl:for-each select="*">
			<xsl:if test="local-name(.) = 'CharacterString' or local-name(.) = 'Anchor'">
